baza_kwiatow = {
    "Nagietek": {
        "typ": "Kwiat Pożyteczny", "ph": "6.0 - 7.0", "swiatlo": "Słoneczne", "woda": "Średnie", "gleba": "Uniwersalna, przepuszczalna",
        "porada": "Wydzieliny jego korzeni niszczą nicienie glebowe. Sadź masowo między warzywami, truskawkami i krzewami owocowymi.",
        "korzystne": ["Pomidor", "Ziemniak", "Truskawka", "Porzeczka", "Agrest"], "niekorzystne": ["Brak"]
    },
    "Aksamitka (Byczek)": {
        "typ": "Kwiat Pożyteczny", "ph": "5.5 - 7.0", "swiatlo": "Słoneczne do półcienia", "woda": "Niskie do średnie", "gleba": "Przeciętna ogrodowa",
        "porada": "Najlepsza tarcza biologiczna w ogrodzie. Odstrasza mączlika szklarniowego, mrówki i krety. Sadź wokół pomidorów, kapusty oraz róż.",
        "korzystne": ["Pomidor", "Kapusta", "Sałata", "Truskawka", "Róża"], "niekorzystne": ["Fasola", "Groch"]
    },
    "Naparstnica": {
        "typ": "Kwiat Pożyteczny", "ph": "5.5 - 6.5", "swiatlo": "Półcień / Cień", "woda": "Średnie", "gleba": "Próchnicza, wilgotna",
        "porada": "Stymuluje wzrost roślin rosnących w jej pobliżu oraz poprawia ich ogólną odporność. Warto sadzić ją na skraju sadu pod koronami drzew owocowych.",
        "korzystne": ["Jabłoń", "Grusza", "Krzewy jagodowe"], "niekorzystne": ["Brak"]
    },
    "Nasturcja": {
        "typ": "Kwiat Pożyteczny", "ph": "6.0 - 7.0", "swiatlo": "Słoneczne", "woda": "Średnie", "gleba": "Uboga (bujniej kwitnie)",
        "porada": "Działa jak 'roślina pułapkowa' – ściąga na siebie wszystkie mszyce, ratując cenne warzywa. Doskonała pod jabłonie, uprawy dyniowate i ziemniaki.",
        "korzystne": ["Dynia", "Cukinia", "Ogórek", "Jabłoń", "Ziemniak"], "niekorzystne": ["Brak"]
    },
    "Słonecznik": {
        "typ": "Kwiat Pożyteczny", "ph": "6.0 - 7.5", "swiatlo": "Pełne słońce", "woda": "Wysokie", "gleba": "Żyzna, głęboko uprawiona",
        "porada": "Tworzy naturalną osłonę wiatrową i rewelacyjną podporę dla pnącej fasoli. Ściąga do ogrodu ptaki żywiące się szkodnikami.",
        "korzystne": ["Fasola pnąca", "Kukurydza", "Ogórek"], "niekorzystne": ["Ziemniak"]
    },
    "Groszek pachnący": {
        "typ": "Kwiat Pożyteczny", "ph": "6.5 - 7.5", "swiatlo": "Słoneczne, osłonięte", "woda": "Średnie", "gleba": "Żyzna, wapienna",
        "porada": "Jak inne rośliny motylkowe, wzbogaca ziemię w azot. Silnie wabi pszczoły. Sadź przy pergolach i w sąsiedztwie warzyw korzeniowych.",
        "korzystne": ["Marchew", "Pietruszka", "Sałata"], "niekorzystne": ["Cebula", "Czosnek"]
    },
    "Konwalia": {
        "typ": "Kwiat Pożyteczny", "ph": "5.0 - 6.0", "swiatlo": "Cień / Głęboki cień", "woda": "Wysokie (stała wilgoć)", "gleba": "Próchnicza, leśna",
        "porada": "Doskonała roślina okrywowa do najciemniejszych, północnych zakątków sadu. Hamuje wzrost chwastów pod starymi krzewami agrestu i porzeczek.",
        "korzystne": ["Agrest", "Porzeczka czarna"], "niekorzystne": ["Warzywa światłolubne"]
    }
}
